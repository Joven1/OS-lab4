...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  stackframe[PROCESS_STACK_PREV_FRAME] = 0; //Error Setting the Stack Frame
  ...
  if (isUser) {
    ...
    //----------------------------------------------------------------------
	stackframe[PROCESS_STACK_USER_STACKPOINTER] = MEM_MAX_VIRTUAL_ADDRESS - 3; //Set as Highest 4-byte aligned address in virtual memory space
    ...
    MemoryCopySystemToUser (pcb, (char *)param, (char *)stackframe[PROCESS_STACK_USER_STACKPOINTER], SIZE_ARG_BUFF);//Error is Here
